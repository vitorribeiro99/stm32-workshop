...
SemaphoreHandle_t uart_dma_lock = 0;

uint8_t buffer[128];
volatile uint8_t UART3Rx_index = 0;
volatile uint8_t outRx_index = 0;
int run = 0;
...
void init_UART3(){
	...
	vSemaphoreCreateBinary(uart_dma_lock);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef* huart){
	if (huart->Instance == USART3){ //current UART?
		...
		outRx_index = UART3Rx_index;
		// set the interrupt for UART3 Rx again
		HAL_UART_Receive_IT(&huart3, &buffer[UART3Rx_index], 1);
	}
}

void HAL_UART_TxCpltCallback(UART_HandleTypeDef* huart){
	if (huart->Instance == USART3){
		xSemaphoreGiveFromISR(uart_dma_lock, pdFALSE);
	}
}

int printDMA(char* str, int length){
	int ret = -1;
	if(xSemaphoreTake(uart_dma_lock,100)){
